...
OUTPUT_FILE = "generated_dataset_final_pipeline.jsonl"
MODEL_ID = "Upstage/SOLAR-10.7B-Instruct-v1.0"
CHUNK_SIZE = 1200

# --- 2. 프롬프트 템플릿 (★★★★★ 파이프라인 방식으로 전면 수정 ★★★★★) ---

# 1단계: 질문과 정답만 생성하는 프롬프트
...
if __name__ == "__main__":
    ...
    if text_chunks:
        ...
        with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
            print(f"\n총 {len(text_chunks)}개의 청크에 대해 QA 생성을 시작합니다...")
            for chunk in tqdm(text_chunks, desc="QA 생성 중"):
                ...
                if not distractor_data or 'distractors' not in distractor_data or len(distractor_data['distractors']) < 3:
                    print(f"\n[실패] 2단계: 오답 생성 실패. 건너뜁니다.")
                    continue
                
                # --- 3단계: 최종 JSON 조립 ---
                options_list = distractor_data['distractors'][:3] + [correct_answer]
                random.shuffle(options_list) # 선택지 순서 섞기
